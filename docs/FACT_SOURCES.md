...
### 4.18 KMS 키 자동 교체 ✅확인됨

**출처**: botocore `kms` `EnableKeyRotation` documentation (2026-07-29 확인).
§5 "확인 못 한 숫자"에 있던 항목을 해소한다.

| 항목 | 값 |
|---|---|
| 고객 관리형 키 자동 교체 주기 | 기본 **약 365일**, `RotationPeriodInDays` 로 조정 가능 |
| AWS 관리형 키 | **매년 자동 교체.** 켜고 끌 수 없다 |
| 주기 변경 이력 | **2022년 5월에 3년 → 1년으로 변경** |
| 지원 대상 | **대칭 암호화 키만.** 비대칭·HMAC·가져온 키 재료·사용자 지정 키 스토어는 제외 |

### 4.19 ACM 인증서 내보내기 ⚠️ 시중 자료가 틀린 항목

**출처**: botocore `acm` `ExportCertificate` documentation (2026-07-29 확인).

시중 자료와 기존 강의가 반복하는
**"ACM 퍼블릭 인증서는 내보낼 수 없어 EC2에 직접 설치할 수 없다"** 는
**더 이상 무조건 참이 아니다.** API 문서가 퍼블릭 인증서 내보내기를 지원한다고 명시하며,
**2025년 6월 17일 이전에 생성된 인증서만 내보낼 수 없다**고 한정한다.

> **문항 작성 주의**: "내보낼 수 없다"를 정답 근거로 쓰는 문항을 만들지 않는다.
> ACM 판단은 **"인증서를 어디에 붙이는가"**(ALB / CloudFront / API Gateway,
> CloudFront 는 `us-east-1` 요구)로 잡는다. 그쪽은 안정적인 사실이다.
...
| ~~계정당 S3 버킷 수 상한~~ | **확인됨 — 범용 버킷 기본 할당량 10,000개** (botocore `s3` `ListBuckets` documentation). 미확인 목록에서 해제 |
...
### 7-0. ⚠️ Snow Family — 라인업이 대폭 축소되었다 (2024~2025)

**출처**: [AWS Snow device updates](https://aws.amazon.com/blogs/storage/aws-snow-device-updates/) (WebSearch 교차 확인, 2026-07-29)

| 장비 | 현재 상태 | 시점 |
|---|---|---|
| **AWS Snowmobile** | **종료** | 2024-03 |
| **AWS Snowcone** | **서비스 중단** | 2024-11-12 |
| 이전 세대 Snowball 3모델 | **단종** | 2024-11-12 |
| **AWS Snowball Edge** | **기존 고객 한정** — 신규 주문 불가 | 2025-11-07 |

> **시험 대비 관점에서 중요하다.**
> 시중 SAA 자료가 여전히 "엑사바이트급은 Snowmobile"이라고 가르치는데,
> **Snowmobile은 이미 없다.** 장비 모델과 용량을 외우는 것은 학습 가치가 사라졌다.
>
> **대신 판단 기준을 가르친다** — "전송량 ÷ 가용 대역폭 = 소요 기간"을 계산해
> 네트워크 전송(DataSync·Direct Connect)으로 감당되는지를 먼저 보고,
> 물리 전송은 그것이 불가능할 때의 선택지로 다룬다.
>
> **문항 에이전트 주의**: "Snowmobile로 엑사바이트를 전송한다" 유형의 문항을 만들지 않는다.
> 특정 장비 모델을 정답으로 요구하는 문항 자체를 피하고, 대역폭·기간 판단을 묻는다.
